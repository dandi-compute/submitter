...
*/1 * * * * /bin/bash -l -c 'flock -n /orcd/data/dandi/001/dandi-compute/submitter/submitter.lock -c "/orcd/data/dandi/001/dandi-compute/submitter/guarded-submit -- sbatch --output /dev/null --error=/dev/null /orcd/data/dandi/001/dandi-compute/submitter/launch_runner.sh" || echo "$(date): lock held, skipping submit"' > /dev/null 2>&1
```



## How to prepare an AIND job (manual)

Use the [Prepare AIND job](https://github.com/dandi-compute/submitter/actions/workflows/prepare-queue.yml) workflow dispatch.

| Input | Description | Default |
|---|---|---|
| `test` | Prepare test queue entries instead of a specific AIND job. | `false` |
| `id` | Content ID to process. Leave blank to prepare from a dandiset clone instead. | _(blank)_ |
| `dandiset` | Dandiset ID to process when `id` is not provided. | `001697` |
| `dandipath` | Local dandiset path to process when `id` is not provided. | `./datalad/dandisets/001697/` |
| `config` | Registered configuration key to use. | `default` |
| `pipeline` | Local path to the AIND pipeline repository. | `./aind-ephys-pipeline.cody/` |
| `version` | Pipeline version to use. Required unless `test` is enabled. | _(blank)_ |
| `params` | Registered pipeline parameter set to use. | `default` |
| `submit` | Automatically submit the prepared job. | `false` |
| `silent` | Suppress command output from `dandicompute prepare aind`. | `false` |
| `queue` | Queue root directory containing `queue_config.json` (used only with `test`). | `./queue/` |
